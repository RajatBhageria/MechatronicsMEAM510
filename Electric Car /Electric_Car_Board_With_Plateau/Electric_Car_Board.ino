...
void loop() {
  ...
  int ADC1 = 500; //set to ADC1 value from the UDP
  ...
  if (motor1ADC > 0 && motor2ADC > 0){
    //first motor 
    //go straight 
    if (motor1ADC > 190 || motor1ADC < 250){
      int toWrite = (int) 220*multiplier; 
      analogWrite(D2,motor1ADC);
      Serial.print("first: ");
      Serial.println(toWrite);
    } else{ //go at the ADC value 
      int toWrite = (int) motor1ADC*multiplier; 
      analogWrite(D2,motor1ADC);
      Serial.print("first: ");
      Serial.println("hey");
    }
    //second motor 
    //go straight 
    if (motor2ADC > 190 || motor2ADC < 250){
      int toWrite = (int) 220*multiplier; 
      analogWrite(D5,motor2ADC);
      Serial.print("second: ");
      Serial.println(motor2ADC);
    } else{ //go at the ADC value 
      //int toWrite = (int) motor2ADC*multiplier; 
      analogWrite(D5,motor2ADC);
      Serial.print("second: ");
      Serial.println("hey");
    }
